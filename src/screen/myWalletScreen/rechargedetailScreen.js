...
const rechargedetailScreen = (props) => {
    ...
    const openPaymentScreen = async () => {
        if (rechargeDetails && rechargeDetails.couponcode) {
            setloading(true);
            try {
                let body = {
                    "txntype": "Cr",
                    "txnref": `Coupon usage - ${rechargeDetails.couponcode}`,
                    "txndate": moment().format(),
                    "customerid": "608153f71f13f33070e41a48",
                    "onModel": "Member",
                    "value": rechargeDetails.property.fixvalue
                }
                console.log(`body`, body);
                const response = await WalletRechargeWithCouponService(body);
                console.log(`response`, response);
                if (response.data != null && response.data != 'undefind' && response.status === 200) {
                    if (Platform.OS === 'android') {
                        ToastAndroid.show('Wallet Recharge Success', ToastAndroid.LONG);
                    } else {
                        alert('Wallet Recharge Success');
                    }
        ...
        <SafeAreaView style={STYLES.rechargeDetailStyles.container}>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
                <View style={{ marginTop: 15, justifyContent: 'center', alignItems: 'center' }}>
                    ...
                    <Text style={{ fontSize: 26, fontWeight: 'bold', color: '#555555' }}>₹ {rechargeDetails && rechargeDetails.couponcode ? rechargeDetails.property.fixvalue : rechargeDetails.amount}</Text>
                </View>
                <View style={{ marginTop: 15, marginLeft: 20 }}>
                    <Text style={{ fontSize: 18, fontWeight: 'bold', color: '#34A853' }}>Recharge Summary</Text>
                </View>
                {
                    rechargeDetails && rechargeDetails.couponcode ?
                                ...
                                <View style={STYLES.rechargeDetailStyles.gamountview}>
                                    <View style={{ marginLeft: 20, marginRight: 20 }}>
                                        ...
                                        <View style={{ justifyContent: 'space-between', flexDirection: 'row', marginTop: 10 }}>
                                            ...
                                            <Text style={{ fontSize: 12, color: '#000000', marginRight: 20 }}> ₹ {rechargeDetails.property.fixvalue}</Text>
                                        </View>
                                        <View style={{ marginTop: 14, flexDirection: 'row' }}>
                                            <View style={{ flex: 1, height: 1, backgroundColor: 'rgba(0, 0, 0, 0.2)' }}></View>
                                        </View>
                                        <View style={{ justifyContent: 'space-between', flexDirection: 'row', marginTop: 5 }}>
                                            <Text style={{ fontSize: 12, marginLeft: 15, color: '#999999' }}>Coupon Applied ({rechargeDetails.property.title})</Text>
                                        ...
                                        <View style={{ justifyContent: 'space-between', flexDirection: 'row', marginTop: 5 }}>
                                            ...
                                            <Text style={{ fontSize: 14, color: '#04DE71', marginRight: 20 }}> ₹ {rechargeDetails.property.fixvalue}</Text>
                                        </View>
                                        <View style={{ marginTop: 14, flexDirection: 'row' }}>
                                            <View style={{ flex: 1, height: 1, backgroundColor: 'rgba(0, 0, 0, 0.3)' }}></View>
                                        </View>
                                ...
                                <View style={{ justifyContent: 'center', alignItems: 'center', marginTop: 5 }}>
                                    <View style={STYLES.rechargePaymentStyles.gamountview}>
                                        ...
                                        <View style={{ justifyContent: 'flex-end', flexDirection: 'row', alignItems: 'center' }}>
                                            <Text style={{ fontSize: 18, color: '#04DE71', marginRight: 20 }}>₹ {rechargeDetails.property.fixvalue}</Text>
